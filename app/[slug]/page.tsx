...
const pages: Record<string, { eyebrow:string; title:string; intro:string; content:React.ReactNode }> = {
  'on-air': {
    eyebrow: 'MEET THE CREW', title: 'On Air', intro: 'Local voices, good music and the kind of radio that feels like New Bern.',
  ...
  contact: {
    ...
    content: <div className="cards"><article className="content-card featured-card"><span className="card-icon">✉️</span><h2>Send us a message</h2><form className="contact-form"><label>Name<input name="name" placeholder="Your name" /></label><label>Email<input type="email" name="email" placeholder="you@example.com" /></label><label>Message<textarea name="message" rows={6} placeholder="What’s on your mind?" /></label><button type="button" className="btn btn-primary">Send Message</button></form><p className="muted">The visual form is in place; message delivery will be connected when we configure the production backend.</p></article><article className="content-card"><span className="card-icon">📍</span><h2>Captain 97 Studios</h2><p><strong>Captain 97.1 · WXNR-LP</strong></p><p>1423 South Glenburnie Road, Suite C<br/>New Bern, NC 28562</p><p><strong>Station Phone</strong><br/><a className="text-link" href="tel:+12526756100">252-675-6100</a></p><p>Listen locally on 97.1 FM or stream online from anywhere.</p><div className="social-stack"><a href="https://www.facebook.com/p/Captain-97-61566324394330/">Facebook ↗</a><a href="https://www.instagram.com/captain97radio/">Instagram ↗</a><a href="https://live365.com/station/Captain-97-a57695">Live365 ↗</a></div></article></div>
  },
};

export function generateStaticParams() { return Object.keys(pages).map(slug => ({slug})); }
export default async function ContentPage({ params }: { params: Promise<{slug:string}> }) { const { slug } = await params; const page = pages[slug]; if (!page) notFound(); return <main className="inner-page"><section className="inner-hero"><div className="container"><div className="eyebrow">{page.eyebrow}</div><h1>{page.title}</h1><p>{page.intro}</p></div></section><section className="section"><div className="container">{page.content}</div></section></main>; }
