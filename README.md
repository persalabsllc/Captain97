...
The STL feed, off-air feed, transmitter readings, transmitter-side RDS, and listener count are populated by an outbound-only station agent. Send a schema-versioned snapshot to `/api/monitoring/ingest` with `Authorization: Bearer <MONITORING_INGEST_TOKEN>`; do not reuse the StationPlaylist token. Each update must include a `sources` object containing only the audio sources that agent actually measures, an explicit transmitter object or `null`, an explicit listener object or `null`, and a current `agentAt` timestamp. An absent audio-source reading remains ready for browser confidence monitoring and is never falsely reported healthy. A representative full payload is:
