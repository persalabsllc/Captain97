...
import { defaults, discoveryCategories, discoverySources, offers, stages, type Dashboard, type DiscoverySource, type Email, type Offer, type Prospect, type Settings } from '@/lib/outreach/types';
import './outreach.css';
  ...
  return <div className="crm crm-app">
    ...
    <main id="main-content" className="crm-main"><header className="crm-topbar"><span>Captain 97.1 / {tab}</span><div><span className={`crm-run-state ${data.settings.enabled ? 'running' : ''}`}>{data.settings.enabled ? 'Automation active' : 'Automation paused'}</span>{data.settings.enabled && <button onClick={() => execute({ action: 'pause' })} disabled={busy}>Pause</button>}<button className="crm-mobile-signout" onClick={logout} disabled={busy}>Sign out</button><button onClick={() => { setError(''); refresh().catch(e => setError(e.message)); }} disabled={busy}>Refresh</button></div></header>
      <div className="crm-content"><div className="crm-heading"><div><p className="crm-kicker">YOUR LOCAL SALES WORKSPACE</p><h1>{tab === 'Overview' ? 'Make the next connection.' : tab}</h1><p>{tab === 'Overview' ? 'A clear view of your outreach, conversations, and opportunities.' : tab === 'Prospects' ? 'Find New Bern businesses and move each conversation forward.' : tab === 'Emails' ? 'Review drafts, manage your queue, and see exactly what was sent.' : 'Set your message, sending pace, and prospect discovery preferences.'}</p></div>{tab !== 'Campaigns' && <button className="crm-primary" disabled={busy} onClick={() => { setTab('Prospects'); setNewProspect(true); setSelected(null); }}>+ Add prospect</button>}</div>
        ...
        {tab === 'Prospects' && <>
          <section className="crm-discover"><div><h2>Find your next local partner</h2><p>Find New Bern businesses across local listings and OpenStreetMap, then check their own websites for published emails.</p></div><form onSubmit={e => { e.preventDefault(); execute({ action: 'discover', category, source }); }}><label htmlFor="research-source">Search source<select id="research-source" value={source} onChange={e => setSource(e.target.value as DiscoverySource)}>{Object.entries(discoverySources).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></label><label htmlFor="research-category">Business category<select id="research-category" value={category} onChange={e => setCategory(e.target.value)}>{discoveryCategories.map(c => <option key={c}>{c}</option>)}</select></label><button className="crm-primary" disabled={busy}>{busy ? 'Working…' : 'Find prospects'}</button></form></section>
          <p className="crm-source-note">All sources includes Visit New Bern, Downtown New Bern, OpenStreetMap, and the Chamber where the category is available. Each search researches up to four new businesses. Map data © <a href="https://www.openstreetmap.org/copyright" target="_blank" rel="noreferrer">OpenStreetMap contributors</a>.</p>
...
function ProspectEditor({ prospect, busy, execute, close, emails, showEmails }: { prospect?: Prospect; busy: boolean; execute: Execute; close: () => void; emails: Email[]; showEmails: () => void }) {
  ...
  return <section className="crm-panel crm-editor"><div className="crm-panel-heading"><h2>{prospect ? 'Prospect details' : 'Add prospect'}</h2><button onClick={close} aria-label="Close prospect details">✕</button></div><form onSubmit={save}>
    ...
    {prospect?.discoveryUrl && <p className="crm-muted">Discovered via <a href={prospect.discoveryUrl} target="_blank" rel="noreferrer">{prospect.discoverySource || 'public listing'} ↗</a></p>}<label>Notes and conversation history<textarea name="notes" rows={5} maxLength={8000} defaultValue={prospect?.notes} placeholder="Calls, replies, goals, or a next step…" /></label><button className="crm-primary" disabled={busy}>Save prospect</button>
